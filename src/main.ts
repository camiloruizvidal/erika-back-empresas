...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Erika Empresas API')
    .setDescription('Documentación de endpoints para gestión de tenants')
    .setVersion('1.0.0')
    .build();
  ...
  logger.log(
    `Aplicación erika-back-empresas corriendo en el puerto ${Config.puerto}`,
  );
}
bootstrap();
